...
from publibot_node.models import ReceivedPublication, VisitorQuestion
...
def publish(request):
    ...
    if existente is not None:
        return JsonResponse(_resposta(existente, status="already_exists"), status=200)
    ...
    return JsonResponse(_resposta(publicacao), status=201)


def _resposta(publicacao: ReceivedPublication, *, status: str = "success") -> dict:
    return {
        "status": status,
